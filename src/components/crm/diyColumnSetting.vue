    ...
    <div class="diyColumnSetting">
      <el-table row-key="prop" class="diyColumnSettingTable" border :data="tableData" style="width: 100%;" ref="multipleTable" v-loading="tableLoading" @select="handleSelect" @select-all="handleSelectAll" @row-contextmenu="handleRowContextMenu">
        ...
        <el-table-column label="字段排序" prop="sortable" align="center">
            <template slot-scope="scope">
                <el-select v-model="scope.row.sortable" placeholder="请选择" :disabled="!scope.row.status||scope.row.prop=='box_in_name'||scope.row.prop=='box_out_name'">
                    <el-option
    ...
    methods:{
        resettingColumn(){
            this.$confirm('是否确认重置列设置并重载页面', '提示', {
                ...
                if(this.pagetype==1){
                    localStorage.setItem('publicCustomerList',JSON.stringify(this.colTemp));
                }else if(this.pagetype==2){
                    localStorage.setItem('privateCustomerList',JSON.stringify(this.colTemp));
                }
                location.reload();
            }).catch(() => {
                this.$message({
