...
<template>
  <div ref="viewer">
    <ul :style="{display: showThumb ? 'block':'none'}" class="vue-viewer-wrapper">
      <li v-for="(item, imgIndex) in list" :key="imgIndex" class="vue-viewer-item">
        <img v-if="typeof item === 'object' && item !== null" :src="item.url" :alt="item.title">
        <img v-else :src="item">
      </li>
    </ul>
  </div>
...
export default {
  ...
  watch: {
    images(newVal) {
      this.list = Array.isArray(newVal) ? newVal : [newVal]
      this.$nextTick(() => {
        this.update()
      })
    },
  ...
  methods: {
    optionsInit() {
      this.list = Array.isArray(this.images) ? this.images : [this.images]
