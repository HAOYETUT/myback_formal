    ...
    <template #header>
      <simple-search :is-float-right="false" :search-columns="searchColumns" @query-search="querySearch">
        <template #prefix>
          ...
          <search-item v-if="pagetype==1">
            <el-select v-model="shop_id" clearable filterable class="w140px" placeholder="全部车队">
              <el-option v-for="item in allCarOptions" :key="item.shop_id" :value="item.shop_id" :label="item.shop_name" />
            </el-select>
          </search-item>
          <search-item>
            <el-select v-model="admin_id" clearable filterable class="w140px" placeholder="所属地推" v-if="pagetype==1">
              <el-option v-for="item in adminOptions" :key="item.admin_id" :value="item.admin_id" :label="item.name" />
            </el-select>
          </search-item>
        </template>
...
export default {
  ...
  methods: {
    ...
    async getCustomList() {
      ...
      // return await monthOrder(requestdata)
    },
    // 统计合计
