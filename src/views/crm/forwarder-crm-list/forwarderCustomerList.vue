    ...
    <el-collapse v-model="activeNames" style="margin-bottom: 20px;">
      <el-collapse-item title="搜索条件" name="searchCondition">
        <el-form :inline="true" class="demo-form-inline">
          <el-form-item label="管车名称:">
            <el-input
              v-model="searchRequest.forwarder_name"
              clearable
          ...
          <el-form-item label="组织代码:">
            <el-input
              v-model="searchRequest.forwarder_id"
              clearable
              prefix-icon="el-icon-search"
              placeholder="请输入组织代码"
              class="inputFrame"
          ...
          <el-form-item label="管车主账号:">
            <el-input
              v-model="searchRequest.mainAccount"
              clearable
              prefix-icon="el-icon-search"
              placeholder="请输入管车主账号"
              class="inputFrame"
            />
          </el-form-item>
      ...
      <el-table border :data="tableData" style="width: 100%;" :row-style="tableRowStyle" ref="multipleTable" @selection-change="handleSelectionChange" @sort-change="sortTableChange" v-loading="customerTableLoading" @row-dblclick="handleRowDblClick" @row-contextmenu="handleRowContextMenu">
        ...
        <el-table-column v-for="(item, index) in col" :key="`col_${index}`" :prop="dropCol[index].prop" :label="item.label" :align="dropCol[index].align" :width="dropCol[index].width==0?true:dropCol[index].width" :sortable="dropCol[index].sortable" :resizable="false">
            <template slot-scope="scope">
              ...
              <span v-else-if="dropCol[index].prop=='forwarder_name'">
                <el-link
                  type="primary"
                  @click="openEditCustomerDrawer(scope.row.id, scope.row.forwarder_name)"
                >{{ scope.row.forwarder_name }}</el-link>
              </span>
              ...
              <span v-else-if="dropCol[index].prop=='forwarder_id'">
                {{scope.row.forwarder_id}}
              </span>
    ...
    <div class="commondialog">
      ...
      >
        <div style="margin: 0 auto; width: 98%;">
          ...
          <p>
            请根据导入模板进行数据导入，红色标题为必填字段。此处"关联账号"字段是为了关联管车列表账号单独设置的，若能查询到对应管车则自动绑定。
          </p>
          <p>
...
<script>
import addOrEditCustomer from '@/components/forwarderCrm/addOrEditCustomer.vue'
import addOrEditContact from '@/components/forwarderCrm/addOrEditContact.vue'
import addOrEditVisitRecord from '@/components/forwarderCrm/addOrEditVisitRecord.vue'
import applyOrCancel from '@/components/forwarderCrm/applyOrCancel.vue'
import diyColumnSetting from '@/components/forwarderCrm/diyColumnSetting.vue'
...
export default {
  ...
  data() {
    return {
      ...
      colTemp:[{label: 'id', prop: 'id',width:80,align:"center",sortable:true,status:true},
          {label: '管车名称', prop: 'forwarder_name',width:180,align:"center",sortable:true,status:true},
          ...
          {label: '组织代码', prop: 'forwarder_id',width:110,align:"center" ,sortable:true,status:true},
      ...
      fileList: [],
      uploadUrl: `${CRM_API}/crm/forwarder/importCustomerExcel`,
  ...
  created() {
    ...
    if (localStorage.getItem('userInfo') != null) {
      ...
      this.userRegion = [];
      if(userInfo.forwarder_region!=null){
        this.userRegion = userInfo.forwarder_region;
      }
  ...
  methods: {
    queryAdminByRegion(){
      ...
      request({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/queryAdminByRegion`,
    ...
    handleHeaderDragend(newWidth, oldWidth, column, event){
      ...
      var columnConfig = '';
      if(this.searchRequest.pagetype==1){
        columnConfig = localStorage.getItem('forwarderPublicCustomerList',JSON.stringify(this.dropCol));
      }else if(this.searchRequest.pagetype==2){
        columnConfig = localStorage.getItem('forwarderPrivateCustomerList',JSON.stringify(this.dropCol));
      }
      ...
      if(this.searchRequest.pagetype==1){
        localStorage.setItem('forwarderPublicCustomerList',JSON.stringify(this.dropCol));
      }else if(this.searchRequest.pagetype==2){
        localStorage.setItem('forwarderPrivateCustomerList',JSON.stringify(this.dropCol));
      }
    },
    ...
    batchDeleteCustomer(){
      ...
      request({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/batchDeleteCustomer`,
        data:requestData
      })
      .then((res) => {
        this.loadTable();
    ...
    confirmBatchAlterDiyField(){
      ...
      request({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/batchAlterDiyFields`,
        data: requestData
      })
      .then((res) => {
        self.loadTable();
    ...
    exportCustomer(){
      ...
      axios({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/exportCustomerExcel`,
        data: requestData,
        responseType: 'blob',
        headers: {
        ...
        .then((res) => {
          ...
          downloadElement.download = '管车CRM客户导出表.xlsx' // 下载后文件名
    ...
    downloadImportTemplate() {
      ...
      axios({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/getImportCustomerExcelTemplate`,
        data: {},
        responseType: 'blob',
        headers: {
          'Content-Type': 'application/json;charset=utf-8',
    ...
    getOperationLogByCustomerId() {
      ...
      request({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/getOperationLogByCustomerId`,
        data: {
    ...
    updateStatusToApplied(id) {
      ...
      request({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/updateStatusToApplied`,
        data: {
          id: id,
    ...
    getVisitRecordListByCustomerId() {
      ...
      request({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/getVisitRecordListByCustomerId`,
        data: {
          customer_id: this.editCustomerId
        }
      })
    ...
    getContactList() {
      ...
      request({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/getContactListByCustomerId`,
        data: {
          customer_id: this.editCustomerId
        }
      })
    ...
    getRegionSelectByEnum() {
      ...
      request({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/getRegionSelectByUserRegion`,
        data: {userRegion:self.userRegion}
      })
        .then((res) => {
    ...
    getCreateUserSelect() {
      ...
      request({
        method: 'post',
        url: `${CRM_API}/crm/forwarder/getCreateUserSelectByUserRegion`,
        data: {userRegion:self.userRegion}
      })
      .then((res) => {
    ...
    loadTable() {
      ...
      requestData.userRegion = self.userRegion
      request({
        url: `${CRM_API}/crm/forwarder/getCustomerList`,
        method: 'post',
        data: requestData
      }).then((res)=>{
    ...
    getPagetype(){
      ...
      if(pagename=='forwarderPublicCustomerList'){
        return 1;
      }else if(pagename=='forwarderPrivateCustomerList'){
        return 2;
      }
    },
  }
