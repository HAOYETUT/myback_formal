...
const excel = {
  ...
  exportExcel: function(data, filename, type, opt) {
    type = type || 'xlsx'
    filename = filename || '导出数据.' + type

    // 创建一个 XLSX工作薄对象
    var wb = XLSX.utils.book_new()
    // 1. 定义excel与工作簿相关的基本属性(设置这些属性后，在 Excel 中点击文件，选择信息，然后点击属性，然后再点击高级属性，最后选择摘要就可以看到)
    var Props = {
      Title: filename, // 标题
      Subject: 'Export From web browser', // 主题
      Author: 'JXT',
      Manager: '',
      Company: '',
      Category: '',
    ...
    opt && opt.Props && (Props = _.extend(Props, opt.Props))
    // 默认进行压缩
    ...
    opt && opt.extend && (wbExtend = _.extend(wbExtend, opt.extend))
    // 清理空配置
    ...
    for (var sheet_name in data) {
      ...
      if (is_aoa) {
        ...
        if (wbExtend[sheet_name]) {
          _.extend(ws, wbExtend[sheet_name])
        } else {
          _.extend(ws, wbExtend)
        }
        // 合并样式
  ...
  numToTitle: function(num) {
    ...
    if (num > 26) {
      ...
      var dec = num % 26
      ans = this.numToTitle((num - dec) / 26) + this.numToTitle(dec || 26)
    ...
    for (var currentRow = startPos.r; currentRow <= endPos.r; currentRow++) {
      for (var currentCol = startPos.c; currentCol <= endPos.c; currentCol++) {
        ...
        if (typeof cell === 'object') {
          newCell = _.merge({}, cell, config)
        } else {
          newCell = _.merge({}, { v: cell }, config)
        }
  ...
  filterDataToAoaData: function(filterData) {
    var aoaData = []
    _.each(filterData, function(index, item) {
      var itemData = []
      for (var i in item) {
        // eslint-disable-next-line no-prototype-builtins
  ...
  filterImportData: function(data, fields) {
    var that = this
    _.each(data, function(fileindex, xlsx) {
      _.each(xlsx, function(sheetname, content) {
  ...
  importExcel: function(files, opt, callback) {
    ...
    if (option.checkMime) {
      _.each(files, function(index, item) {
  ...
  // 格式化title标题
  formateTitle: function(arr = [], key = 'custom_seq_title') {
    return arr.map(el => {
      const obj = {}
